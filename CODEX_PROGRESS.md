...
- 分支：`main`；`0.2.7` 补丁发布准备中，历史正式标签保持固定，不移动已发布标签。
...
- 完成 `0.2.7` 更新结果异常反馈验证、本地 Lite 预览和回归测试后，提交、推送标签并等待 GitHub Actions 发布。
- 发布后逐项核对四种包、三份清单和 `SHA256SUMS.txt`，使用客户端内置公钥实际验签线上 Lite/Full 清单，并确认 latest 指向 `v0.2.7`。
...
> 当前发布准备版本：`0.2.7`；`v0.2.3` 因 ReleaseTool 编译失败未发布，历史发布记录保持不变。
