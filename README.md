...
当前发布版本：`0.5.0`。请从 [GitHub Releases](https://github.com/Kratosmax/PathEcho/releases/latest) 下载对应版本资产。

<img src="src/PathEcho/Assets/PathEchoLogo.png" alt="PathEcho Logo" width="96" />

PathEcho 是面向 Windows 的本机目录同步与游戏存档版本备份工具。它提供同步预演与过滤、运行记录、内容寻址去重快照、事务回档、远程游戏识别规则、低开销目录监听和签名自动更新。
...
| 包 | 适用场景 |
|---|---|
| `PathEcho-0.5.0-Full-Setup.exe` | 推荐，自带 .NET 8 运行时并提供卸载程序 |
| `PathEcho-0.5.0-Lite-Setup.exe` | 安装版，需要 [.NET 8 Desktop Runtime x64](https://dotnet.microsoft.com/download/dotnet/8.0) |
| `PathEcho-0.5.0-Full.zip` | 免安装，自带 .NET 8 运行时 |
| `PathEcho-0.5.0-Lite.zip` | 最小免安装包，需要 .NET 8 Desktop Runtime x64 |
...
2. 在“游戏存档”中设置存档目录、备份目录、最近/每小时/每日三层保留策略和定时/文件变化/游戏退出触发；双击现有游戏即可编辑，可手动选择运行进程，也可用 GitHub 规则智能识别已启动游戏。
3. 在“版本历史”中按游戏筛选或搜索存档快照；双击版本可查看其中每个文件的文件名、所在目录、修改时间和精确大小。“打开”会生成可自由修改的独立浏览副本，不会改坏历史版本。也可浏览最近 200 次同步运行结果，并按完整目录、变动文件或正则匹配文件事务回档。
4. 在“设置”中管理当前用户开机自启、后台启动、默认备份目录、自动备份通知、多条更新线路、HTTP 出网代理和 Debug 日志。自动备份通知支持深色/浅色、目标显示器、四角位置及每个位置独立的 X/Y 微调；每个游戏可继承默认、关闭或单独设置。
...
游戏存档正常备份遇到临时读写失败时，会先把完整且校验通过的存档副本放到默认备份目录的 `temp\<游戏ID>\<事务ID>`，再从该副本向正式备份位置重试。读源、写入和旧版本清理均每 5 秒重试；每连续失败 10 次会询问是否继续。停止写入重试或程序退出时会保留已经完整落盘的临时副本并报告路径，成功后会清理本次临时目录。若默认备份目录本身不可写或磁盘已满，则无法建立本地保底副本。

快照以“完整逻辑清单 + SHA-256 对象库”保存：每个版本都能独立回档，相同内容只存一份，文件变化触发只重新读取变化路径。重点文件触发、定时备份、游戏退出和监听溢出仍执行整目录快照。旧版 `files` 快照继续可读；只有在所有对象逐项验真后才会移除旧硬链接视图并升级清单，坏对象会回退到仍有效的旧视图。迁移或导入备份时按清单复制唯一对象并校验，不重复复制每个版本的文件树。

自动清理会保留“最近版本”、每个小时最新的一份锚点和每天最新的一份锚点，三组结果取并集，同一快照不会重复占用。旧配置中的“保留版本”数值继续作为“最近版本”使用；每小时和每日锚点可设为 `0` 单独关闭。只有不再被任何保留快照引用的内容对象才会被清理。

编辑游戏时若改变了该游戏的单独备份目录，PathEcho 会先迁移并校验现有版本，再保存配置；保存失败会尝试回滚迁移。仅修改名称、触发方式、进程或保留数量时，不移动已有备份。

自动备份成功或最终失败时会显示无焦点置顶小窗，手动备份不会重复弹窗。连续相同错误会合并计数，通知队列最多保留 4 条并在 5 秒后自动消失；坐标始终限制在所选显示器工作区内。
...
`v0.3.0` 及更早版本若自动更新提示安装目录被占用或“下载完成后程序退出且没有后续”，旧客户端无法通过下载新包修复自身正在执行的更新器，请从 Release 页面手动覆盖安装当前版本一次。配置和备份不会被覆盖安装删除；`0.3.1` 及更高版本可使用修复后的自动更新链。
