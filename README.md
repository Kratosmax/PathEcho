...
| 包 | 适用场景 |
|---|---|
| `PathEcho-0.2.1-Full-Setup.exe` | 推荐，自带 .NET 8 运行时并提供卸载程序 |
| `PathEcho-0.2.1-Lite-Setup.exe` | 安装版，需要 [.NET 8 Desktop Runtime x64](https://dotnet.microsoft.com/download/dotnet/8.0) |
| `PathEcho-0.2.1-Full.zip` | 免安装，自带 .NET 8 运行时 |
| `PathEcho-0.2.1-Lite.zip` | 最小免安装包，需要 .NET 8 Desktop Runtime x64 |
...
1. 在“同步任务”中新建目录同步，选择单向、反向或双向，以及删除和冲突策略；任务页支持搜索、状态筛选、目录可用性提示、双击编辑和右键快捷操作。
