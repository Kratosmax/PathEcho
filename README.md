...
当前发布版本：`0.4.0`。请从 [GitHub Releases](https://github.com/Kratosmax/PathEcho/releases/latest) 下载对应版本资产。
...
| 包 | 适用场景 |
|---|---|
| `PathEcho-0.4.0-Full-Setup.exe` | 推荐，自带 .NET 8 运行时并提供卸载程序 |
| `PathEcho-0.4.0-Lite-Setup.exe` | 安装版，需要 [.NET 8 Desktop Runtime x64](https://dotnet.microsoft.com/download/dotnet/8.0) |
| `PathEcho-0.4.0-Full.zip` | 免安装，自带 .NET 8 运行时 |
| `PathEcho-0.4.0-Lite.zip` | 最小免安装包，需要 .NET 8 Desktop Runtime x64 |
...
2. 在“游戏存档”中设置存档目录、备份目录、保留版本数和定时/文件变化/游戏退出触发；双击现有游戏即可编辑，可手动选择运行进程，也可用 GitHub 规则智能识别已启动游戏。
3. 在“版本历史”中按游戏筛选或搜索存档快照；“打开”会生成可自由修改的独立浏览副本，不会改坏历史版本。也可浏览最近 200 次同步运行结果，并按完整目录、变动文件或正则匹配文件事务回档。
